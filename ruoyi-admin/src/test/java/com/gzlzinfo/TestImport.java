...
public class TestImport {
    ...
    private static final String PREFIX_PATH = "/Users/yan/work/docs/10中大科研辅助系统/data_20230614/";
...
//        importCertificate("证书表.csv");
    ...
    private void importExpert(String fileName) {
        ...
        String sql = "insert into `kyfz_expert` (`expert_id`, `expert_account`, `expert_name`, `expert_position`, `expert_affiliation`," +
                ...
                " `certificate_id`, `work_id`, `requirement_id`, `expert_teams`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
        for (String[] el : list) {
            jdbcTemplate.update(sql, el[0], el[1], el[2], el[3], el[4], el[5], el[6], el[7], el[8], el[9], el[10], el[11], el[12], el[13], el[14]);
//            KyfzExpert expert = new KyfzExpert();
//            expert.setExpertId(Long.parseLong(el[0]));
    ...
    private void importRequirements(String fileName) {
        ...
        for (String[] el : list) {
            ...
            jdbcTemplate.update(sql, el[0], el[1], el[2], el[3], el[4], el[5], el[6], el[7], el[8], el[9], el[10],
                    el[11], el[12], el[13], el[14], el[15], date, date1, el[18], el[19], el[20], el[21], el[22], el[23], el[24], el[25]);
...
//            requirementService.insertKyfzRequirement(requirement);
    ...
    private void importWork(String fileName) {
        ...
        jdbcTemplate.execute(SQL_MODE_SQL);
        for (String[] el : list) {
            jdbcTemplate.update(sql, el[0], el[1], el[2], el[3], el[4], el[5], el[6], el[7], el[8], el[9], el[10], el[11], el[12], el[13], el[14], el[15], el[16]);
        }
    }
    ...
    private void importCertificate(String fileName) {
        ...
        for (String[] el : list) {
            ...
            jdbcTemplate.update(sql, el[0], el[1], el[2], el[3], el[4], date, date1, el[7], el[8], el[9], el[10], el[11], el[12], el[13], el[14], el[15]);
        }
    }
